...
​```
label 뿐만아니라 unlabel을 이용해서 분포된함수의 parameter를 찾는데 정확성을 높이는 알고리즘총 3개의 library를 사용했다
...
최초에 난수생성했을때의다각각 설정한 파라미터와 추정된 파라미터값이 얼마나 가까운지 비교했다
em이 결국 (global optimal일수도있지만 확인이 불가하므로) local optimal을 찾는 것이므로, 공개된데이터가 아니라, exact parameter를 갖는 data로 em을 돌리고 실제 exact para값과 추정값이 얼마나 차이가 있는지 확인하려고 난수생성데이터를 활용했다.
...
for i in range(100):
	labeldata1[N2[0][i],N2[1][i]]='red'
​```
labeled data 생성:label을 붙이므로 dictionary형태로 labeled data숫자가 작으면 parameter의 정확도가 떨어지는 걸 확인할 수 있었다. 어느정도의 수는 보장해야한다고 할 수 있다. 장점은 em알고리즘을 사용하면, gaussain 에서 local optimal에 빠질수있는 위험이 있지만, generative model은 이미 label데이터로 local optimal을 찾고 반복을 시작하기 때문에, 상대적으로 local optimal에 빠질 가능성이 상대적으로 gmm보다 낮을것으로 예상된다.
...
label, unlabeled data 시각화.작은 점들이 nonlabeld 파란점 빨간점은 labeld data.
...
데이터가 어떻게 분포되었는지의, 시각화를 위한 label, unlabeled data 키값 축출
좌표에 해당되는 key값을 추출
​```python
